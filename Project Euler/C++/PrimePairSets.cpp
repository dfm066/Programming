...
bool match(N a, N b ) {
    ...
    if (is_prime(ab, 5) && is_prime(ba, 5)) return true;
    return false;
}
...
int main() {
    ...
    for (auto first = begin(primes); first != end(primes); first++) {
        for (auto second = next(first); second != end(primes); second++) {
            if (!match(*first, *second)) continue;
            for (auto third = next(second); third != end(primes); third++) {
                if (!match(*first, *third) || !match(*second, *third)) continue;
                // cout << "Found 3 : [" << *first << ", " << *second << ", " << *third << "]" << endl;
                for (auto fourth = next(third); fourth != end(primes); fourth++) {
                    if (!match(*first, *fourth) || !match(*second, *fourth) || !match(*third, *fourth)) continue;
                    //  cout << "Found 4 : [" << *first << ", " << *second << ", " << *third << ", " << *fourth << "]" << endl;
                    for (auto fifth = next(fourth); fifth != end(primes); fifth++) {
                        if (!match(*first, *fifth) || !match(*second, *fifth) || !match(*third, *fifth) || !match(*fourth, *fifth)) continue;
    ...
    cout << "[";
    for (const auto& val: begin(summation)->second) cout << val << ", ";
    cout << "\b\b]" << endl; 
